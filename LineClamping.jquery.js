$(function() {
   ...
   if (!supports("line-clamp", 2) {
     var $clamped = $("[data-clamp]");

     $.each($clamped, function(clamp, i) {
      var $clamp = $(clamp);
      var maxLines = Math.max(parseInt($clamp.attr("data-clamp"), 10), 1);

      if (isNaN(maxLines)) {
       maxLines = 1;
      }

      var lineHeight = Math.round(parseFloat(window.getComputedStyle($clamp[0]).lineHeight));
      $clamp.css("max-height", lineHeight * maxLines + "px");
     });
    }
